					  ...
					  bool keep_input_buffer /* = false */)
{
	...
	imp_check(header_entries[0] == "P5" || header_entries[0] == "P6");
	...
	imp_check(!is_texture || (width & (width - 1)) == 0);
	imp_check(!is_texture || (height & (height - 1)) == 0);
	
	// Read color scale
    unsigned int max_color_value = (unsigned int)(atoi(header_entries[3].c_str()));
	imp_check(max_color_value == 255);

	int color_offset;
	imp_float color_norm;

	imp_check(range_type == 0 || range_type == 1);
...
{
	imp_check(data);
	imp_check(n_components == 1 || n_components == 3);
